    ...
    <ScrollArea className="h-[300px] w-full rounded-md border p-4 bg-card">
      <div className="space-y-4">
        {commitments.map((item) => (
          <div key={item.id} className="flex items-start gap-3 pb-4 border-b last:border-0 last:pb-0" data-testid={`commitment-item-${item.id}`}>
            ...
            <div className="flex-1 space-y-1">
              ...
              <div className="flex items-center text-xs text-muted-foreground gap-2">
                <span className="font-medium">{item.meetingTitle ?? "No meeting"}</span>
                <span>•</span>
