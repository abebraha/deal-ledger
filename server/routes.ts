...
export async function registerRoutes(
  ...
  app.options("/api/migrate-data", (req, res) => {
  ...
  app.post("/api/migrate-data", async (req, res) => {
    ...
    try {
      ...
      // Accepts: { accountName, data: { deals, activities, meetings, firefliesMeetings, settings, salesReps, reports, connections } }
      const { accountName, data } = req.body as {
        accountName: string;
        data: {
          deals?: any[];
          activities?: any[];
          meetings?: any[];
          firefliesMeetings?: any[];
          settings?: any[];
          salesReps?: any[];
          reports?: any[];
          connections?: { hubspot?: any; fireflies?: any; close?: any };
        };
      };

      if (!accountName || !data) {
        return res.status(400).json({ error: "accountName and data are required" });
      }

      const results: Record<string, number> = {
        accounts: 0, deals: 0, activities: 0, meetings: 0,
        firefliesMeetings: 0, settings: 0, salesReps: 0, reports: 0, connections: 0
      };

      // 1. Create account
      const [newAcc] = await db.insert(schema.accounts).values({ name: accountName }).returning();
      const newAccountId = newAcc.id;
      results.accounts = 1;

      // 2. Settings
      for (const s of data.settings || []) {
        await db.insert(schema.settings).values({ accountId: newAccountId, key: s.key, value: s.value });
        results.settings++;
      }

      // 3. Sales reps
      for (const sr of data.salesReps || []) {
        await db.insert(schema.salesReps).values({
          accountId: newAccountId,
          name: sr.name,
          hubspotOwnerId: sr.hubspotOwnerId ?? null,
          closeUserId: sr.closeUserId ?? null,
          excluded: sr.excluded ?? false,
        });
        results.salesReps++;
      }

      // 4. Connections (insert with whatever config we have)
      ...
      // 5. Deals — build old-id → new-id map
      const dealMap: Record<number, number> = {};
      for (const deal of data.deals || []) {
        const [newDeal] = await db.insert(schema.deals).values({
          accountId: newAccountId,
          hubspotId: deal.hubspotId ?? null,
          name: deal.name,
          ...
          closeUrl: deal.closeUrl ?? null,
          pipeline: deal.pipeline ?? null,
        }).returning();
        dealMap[deal.id] = newDeal.id;
        results.deals++;
      }

      // 6. Activities (batched)
      const acts = data.activities || [];
      for (let i = 0; i < acts.length; i += 200) {
        const batch = acts.slice(i, i + 200);
        if (batch.length === 0) break;
        await db.insert(schema.activities).values(batch.map((a: any) => ({
          accountId: newAccountId,
          hubspotId: a.hubspotId ?? null,
          dealId: a.dealId ? (dealMap[a.dealId] ?? null) : null,
          type: a.type,
          ...
          hubspotUrl: a.hubspotUrl ?? null,
          closeId: a.closeId ?? null,
          closeUrl: a.closeUrl ?? null,
        })));
        results.activities += batch.length;
      }

      // 7. Meetings (batched)
      const mtgs = data.meetings || [];
      for (let i = 0; i < mtgs.length; i += 200) {
        const batch = mtgs.slice(i, i + 200);
        if (batch.length === 0) break;
        await db.insert(schema.meetings).values(batch.map((m: any) => ({
          accountId: newAccountId,
          hubspotId: m.hubspotId ?? null,
          dealId: m.dealId ? (dealMap[m.dealId] ?? null) : null,
          title: m.title ?? null,
          ...
          hubspotUrl: m.hubspotUrl ?? null,
          closeId: m.closeId ?? null,
          closeUrl: m.closeUrl ?? null,
        })));
        results.meetings += batch.length;
      }

      // 8. Fireflies meetings (batched)
      const fms = data.firefliesMeetings || [];
      for (let i = 0; i < fms.length; i += 50) {
        const batch = fms.slice(i, i + 50);
        if (batch.length === 0) break;
        await db.insert(schema.firefliesMeetings).values(batch.map((fm: any) => ({
          accountId: newAccountId,
          firefliesId: fm.firefliesId ?? null,
          title: fm.title ?? null,
          meetingDate: fm.meetingDate ?? null,
          duration: fm.duration ?? null,
          participants: fm.participants ?? null,
          summary: fm.summary ?? null,
          outline: fm.outline ?? null,
          keywords: fm.keywords ?? null,
          transcript: fm.transcript ?? null,
        })));
        results.firefliesMeetings += batch.length;
      }
      ...
      res.json({ success: true, newAccountId, results });
    } catch (err: any) {
